...
public class InGameInterval : GameSection
{
	...
	private unsafe IEnumerator DoInitialize()
	{
		...
		Action<bool> matching_end_action = delegate(bool is_connect)
		{
			if (!is_connect)
			{
				((_003CDoInitialize_003Ec__IteratorCD)/*Error near IL_0206: stateMachine*/)._003C_003Ef__this.coopServerInvalidFlag = true;
			}
			else
			{
				((_003CDoInitialize_003Ec__IteratorCD)/*Error near IL_0206: stateMachine*/)._003Cmatching_flag_003E__5 = true;
			}
		};
		if (MonoBehaviourSingleton<InGameManager>.I.isTransitionFieldToQuest)
		{
			...
			else if (MonoBehaviourSingleton<InGameManager>.I.isQuestHappen)
			{
				...
				{
					((_003CDoInitialize_003Ec__IteratorCD)/*Error near IL_0333: stateMachine*/)._003Cmatching_end_action_003E__6(CoopWebSocketSingleton<KtbWebSocket>.IsValidConnected());
				});
