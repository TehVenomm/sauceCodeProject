public class InGameDeliveryDetailBase : QuestDeliveryDetail
{
	...
	private void OnScreenRotate(bool is_portrait)
	{
		if (isUpdatedUI)
		{
			if (is_portrait)
			{
				SetActive(UI.CHARA_ALL, is_visible: true);
				SetActive(UI.PORTRAIT_BACK, is_visible: true);
				SetActive(UI.LANDSCAPE_BACK, is_visible: false);
			...
			else
			{
				...
				SetActive(UI.PORTRAIT_BACK, is_visible: false);
				SetActive(UI.LANDSCAPE_BACK, is_visible: true);
