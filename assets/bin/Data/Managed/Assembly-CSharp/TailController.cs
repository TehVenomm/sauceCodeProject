	...
	private void LateUpdate()
	{
		if (isUpdate)
		{
			...
			for (int i = 1; i < pointList.Length; i++)
			{
				...
				Transform transform2 = pointList[i];
				...
				float angle = Mathf.Min(Vector3.Angle(normalized, normalized2), angleMax);
				Quaternion quaternion = Quaternion.AngleAxis(angle, Vector3.Cross(normalized, normalized2));
		...
		else if (m_finishLerpTime > 0f)
		{
			...
			{
				Transform transform3 = pointList[j];
				transform3.localRotation = Quaternion.Lerp(m_lerpRotationList[j], transform3.localRotation, num2);
				transform3.localPosition = Vector3.Lerp(m_lerpPositionList[j], transform3.localPosition, num2);
			}
			if (num2 >= 1f)
			{
				m_finishLerpTime = 0f;
