		...
		if (!(animEventData == null))
		{
			...
			for (int num = animations.Length; i < num; i++)
			{
				...
				for (int num2 = events.Length; j < num2; j++)
				{
					...
					{
						string text6 = eventData.stringArgs[0];
						if (name_analyzer != null)
						{
							text6 = name_analyzer(text6);
						}
						if (!string.IsNullOrEmpty(text6))
						{
							CacheEffect(RESOURCE_CATEGORY.EFFECT_ACTION, text6);
						}
						break;
					}
					...
					{
						int k = 1;
						for (int num5 = eventData.stringArgs.Length; k < num5; k++)
						{
					...
					case AnimEventFormat.ID.WEAKPOINT_ON:
					{
						...
						string text2 = (eventData.stringArgs.Length <= 0) ? string.Empty : eventData.stringArgs[0];
						if (!string.IsNullOrEmpty(text2))
						{
							if (Enum.IsDefined(typeof(EQUIPMENT_TYPE), text2))
							{
								EQUIPMENT_TYPE equipment_type = (EQUIPMENT_TYPE)(int)Enum.Parse(typeof(EQUIPMENT_TYPE), text2);
								eventData.attackMode = Player.ConvertEquipmentTypeToAttackMode(equipment_type);
							}
							else
							{
								Log.Error("Undefined EQUIPMENT_TYPE name:" + text2);
							}
						}
						...
						if (Enemy.IsWeakStateElementAttack(wEAK_STATE) || Enemy.IsWeakStateSkillAttack(wEAK_STATE) || Enemy.IsWeakStateHealAttack(wEAK_STATE) || Enemy.IsWeakStateCannonAttack(wEAK_STATE))
						{
							...
							{
								string text3 = MonoBehaviourSingleton<InGameSettingsManager>.I.targetMarkerSettings.effectNames[(int)eFFECT_TYPE];
								switch (wEAK_STATE)
								{
								...
								case Enemy.WEAK_STATE.WEAK_ELEMENT_SP_ATTACK:
								{
									...
									if (eventData.attackMode != 0 && num3 >= 0)
									{
										text3 = string.Format(text3, (int)(eventData.attackMode - 1), num3);
									}
									break;
								}
								}
					...
					{
						eventData.attackMode = Player.ATTACK_MODE.NONE;
						string text5 = (eventData.stringArgs.Length <= 0) ? string.Empty : eventData.stringArgs[0];
						if (!string.IsNullOrEmpty(text5))
						{
							if (!Enum.IsDefined(typeof(EQUIPMENT_TYPE), text5))
							{
								Log.Error("Undefined EQUIPMENT_TYPE name:" + text5);
							}
							else
							{
								EQUIPMENT_TYPE equipment_type2 = (EQUIPMENT_TYPE)(int)Enum.Parse(typeof(EQUIPMENT_TYPE), text5);
