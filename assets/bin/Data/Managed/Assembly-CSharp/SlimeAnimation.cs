...
public class SlimeAnimation
{
	...
	public void Update()
	{
		...
		{
			Color color2 = slimeMaterial.color;
	...
	{
		posAnimator.SetAnimation(SLIME_ANIM_CURVE_ZERO, 1f, true, slimeTransform.localPosition, CallBackPos);
		scaleAnimator.SetAnimation(slime.animFadeIn, slime.fadeInAnimTime, true, slimeTransform.localScale, CallBackScale);
		colorAnimator.SetAnimation(AnimationCurve.Linear(0f, 0f, 1f, 0.5f), slime.fadeInColorAnimTime, false, slimeMaterial.color, CallBackColor);
		isFadeOut = false;
	}

	public void TouchOff(Action CallBackPos = null, Action CallBackScale = null, Action CallBackColor = null)
	{
		posAnimator.SetAnimation(slime.animFadeOut, slime.fadeOutAnimTime, true, slimeTransform.localPosition, CallBackPos);
		scaleAnimator.SetAnimation(SLIME_ANIM_CURVE_ONE, 1f, true, slimeTransform.localScale, CallBackScale);
		colorAnimator.SetAnimation(AnimationCurve.Linear(0f, 0.5f, 1f, 0f), slime.fadeOutColorAnimTime, true, slimeMaterial.color, CallBackColor);
		isFadeOut = true;
	}

	public void Crush(Action CallBackPos = null, Action CallBackScale = null, Action CallBackColor = null)
	{
		posAnimator.SetAnimation(SLIME_ANIM_CURVE_ZERO, 1f, false, slimeTransform.localPosition, CallBackPos);
		scaleAnimator.SetAnimation(slime.animCrush, slime.crushAnimTime, true, slimeTransform.localScale, CallBackScale);
		colorAnimator.SetAnimation(AnimationCurve.Linear(0f, 0.5f, 1f, 0f), slime.crushColorAnimTime, false, slimeMaterial.color, CallBackColor);
		isFadeOut = true;
	}
	...
	{
		posAnimator.SetAnimation(SLIME_ANIM_CURVE_ZERO, 1f, true, slimeTransform.localPosition, CallBackPos);
		scaleAnimator.SetAnimation(AnimationCurve.Linear(0f, 1f, slime.scaleupAnimTime, slime.scaleupAnimMaxScale), slime.scaleupAnimTime, true, slimeTransform.localScale, CallBackScale);
		colorAnimator.SetAnimation(SLIME_ANIM_CURVE_HALF, 1f, false, slimeMaterial.color, CallBackColor);
		isFadeOut = false;
	}
	...
	{
		posAnimator.SetAnimation(SLIME_ANIM_CURVE_ZERO, 1f, true, slimeTransform.localPosition, CallBackPos);
		scaleAnimator.SetAnimation(slime.animScaleUpDown, slime.scaleUpDownAnimTime, true, slimeTransform.localScale, CallBackScale);
		colorAnimator.SetAnimation(SLIME_ANIM_CURVE_HALF, 1f, false, slimeMaterial.color, CallBackColor);
		isFadeOut = false;
	}
}
