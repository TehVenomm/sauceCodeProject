...
public class QuestAcceptArenaDetail : QuestDeliveryDetail
{
	...
	public override void Initialize()
	{
		...
		ResourceLoad.LoadWithSetUITexture(GetCtrl(UI.TEX_RUSH_IMAGE).GetComponent<UITexture>(), RESOURCE_CATEGORY.ARENA_RANK_ICON, ResourceName.GetArenaRankIconName(arenaData.rank));
	...
	public override void UpdateUI()
	{
		...
		SetActive(UI.BTN_COMPLETE_RANK_UP, is_visible: false);
		base.UpdateUI();
		SetActive(UI.BTN_CREATE_OFF, is_visible: false);
		UpdateTime();
	...
	private IEnumerator PlayRankUpEffect(bool is_unlock_portal, string effectName)
	{
		...
		SetActive(UI.OBJ_BASE_FRAME, is_visible: false);
		SetActive(UI.BTN_COMPLETE_RANK_UP, is_visible: true);
