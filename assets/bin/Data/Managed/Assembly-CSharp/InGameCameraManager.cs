...
public class InGameCameraManager : MonoBehaviourSingleton<InGameCameraManager>
{
	...
	private void Start()
	{
		...
		if (ctrlCamera.gameObject.GetComponent<InGameCameraCuller>() == null)
		{
			ctrlCamera.gameObject.AddComponent<InGameCameraCuller>();
		}
	}
	...
	private void UpdateRadialBlur()
	{
		...
		zero = ((!(radialBlurCenterTransform != null)) ? radialBlurCenterPos : radialBlurCenterTransform.position);
	...
	private void UpdatePlayerCamera()
	{
		...
		if (!isFixedCameraMode)
		{
			if (isMotionCameraMode)
			{
				...
				if (motionCameraTransforms != null)
				{
					...
					requestPos = motionCameraTransforms[num6].position;
					requestRot = motionCameraTransforms[num6].rotation;
					float num7 = motionCameraTransforms[num6].localScale.x;
					if (num7 > 0f)
					{
			...
			else if (!flag || !validSettings.targetEnable)
			{
				...
				Vector3 vector2 = Quaternion.AngleAxis(0f - validSettings.freePitch, axis) * back * Mathf.Lerp(validSettings.distanceLimit, num5, num4) * num8;
				Vector3 b = Quaternion.LookRotation(back) * validSettings.freeOffset * num8;
				...
				if (targetPlayer.actionID == Character.ACTION_ID.ATTACK && !targetPlayer.isArrowAimLesserMode && targetPlayer.attackStartTarget != null && targetPlayer.attackStartTarget == targetPlayer.actionTarget && MonoBehaviourSingleton<TargetMarkerManager>.I.isTargetLock)
				{
					...
					if (vector4.z < (0f - validSettings.followBackOffset) * num8)
					{
						zero.z = vector4.z + validSettings.followBackOffset * num8;
					}
					Vector3 vector5 = Quaternion.FromToRotation(vector3, Vector3.forward) * (vector3 + vector4);
					float num9 = Mathf.Tan((float)Math.PI / 180f * fieldOfView * 0.5f) * ((float)Screen.width / (float)Screen.height);
					float num10 = Mathf.Abs(vector5.x / vector5.z) / num9;
					if (num10 > validSettings.followSidePercent)
					{
						zero.x = vector4.x * (num10 - validSettings.followSidePercent) / num10;
					}
					if (back != Vector3.forward)
					{
			...
			else
			{
				...
				Vector3 vector = enemy.GetCameraTargetPos();
				...
				{
					float num11 = Vector3.Angle(vector6, vector7);
					if (num11 > validSettings.moveableTargetAngle)
					{
						float num12 = (Vector3.Cross(vector7, vector6).y >= 0f) ? 1f : (-1f);
						normalForward = Quaternion.AngleAxis((num11 - validSettings.moveableTargetAngle) * num12, Vector3.up) * vector7;
					}
				}
				...
				{
					float time = Mathf.Clamp01(((vector - cameraTargetPos).magnitude - validSettings.targetingPitchNearDistance) / (validSettings.targetingPitchFarDistance - validSettings.targetingPitchNearDistance));
					time = validSettings.targetingPitchCurve.Evaluate(time);
					num13 = validSettings.targetingPitchMinAngle * (1f - time) + validSettings.targetingPitchMaxAngle * time;
				}
				Vector3 axis2 = Vector3.Cross(normalForward, Vector3.up);
				Vector3 b3 = Quaternion.AngleAxis(0f - num13, axis2) * normalForward * Mathf.Lerp(validSettings.distanceLimit, num5, num4);
				...
				if (validAnimEventTargetOffset != null)
				{
					...
					num2 = ((validAnimEventTargetOffset.smoothMaxSpeed > 0f) ? validAnimEventTargetOffset.smoothMaxSpeed : num2);
				}
				...
				{
					requestRot = Quaternion.LookRotation(validAnimEventTargetPosition.pos - cameraTransform.position);
					num3 = ((validAnimEventTargetPosition.smoothMaxSpeed > 0f) ? validAnimEventTargetPosition.smoothMaxSpeed : num3);
				}
			}
		}
		if (hitObjectType != 0 && !isMotionCameraMode)
		{
			Vector3 vector8 = requestPos - cameraTargetPos;
			RaycastHit hitInfo = default(RaycastHit);
			Ray ray = new Ray(cameraTargetPos, vector8.normalized);
			if (Physics.Raycast(ray, out hitInfo, num5, 2359808) && hitObjectType == CAM_HIT_OBJ_TYPE.ZOOM)
			{
				float num14 = hitInfo.distance;
				if (num14 < validSettings.distanceLimit)
				{
					num14 = validSettings.distanceLimit;
				}
				num14 -= validSettings.distanceLimit;
				float num15 = num5 - validSettings.distanceLimit;
				float num16 = 0f;
				if (num15 > 0f)
				{
					num16 = validSettings.distanceLerpTime * (num14 / num15);
				}
				if (modeChangeTime <= 0f)
				{
					distanceElapsedTime -= Time.deltaTime * 5f;
					if (distanceElapsedTime < num16)
					{
						distanceElapsedTime = num16;
					}
				}
				else
				{
				...
				requestPos = cameraTargetPos + ray.direction * Mathf.Lerp(validSettings.distanceLimit, num5, num4);
			}
		}
		if (num != 0f && modeChangeTime <= 0f)
		{
			num *= ((num4 < 0.1f) ? 0.1f : num4);
		}
		if (switching)
		{
		...
		{
			Vector3 movePosition2 = Vector3.SmoothDamp(movePosition, requestPos, ref posVelocity, num, num2, Time.deltaTime);
			Vector3 zero2 = Vector3.zero;
			Vector3 eulerAngles = moveRotation.eulerAngles;
			Vector3 eulerAngles2 = requestRot.eulerAngles;
			zero2.x = Mathf.SmoothDampAngle(eulerAngles.x, eulerAngles2.x, ref rotVelocity.x, num, num3, Time.deltaTime);
			zero2.y = Mathf.SmoothDampAngle(eulerAngles.y, eulerAngles2.y, ref rotVelocity.y, num, num3, Time.deltaTime);
			zero2.z = Mathf.SmoothDampAngle(eulerAngles.z, eulerAngles2.z, ref rotVelocity.z, num, num3, Time.deltaTime);
			Quaternion identity = Quaternion.identity;
			identity.eulerAngles = zero2;
			this.movePosition = movePosition2;
			this.moveRotation = identity;
			fieldOfView = Mathf.SmoothDamp(ctrlCamera.fieldOfView, fieldOfView, ref fieldOfViewVelocity, num, num3, Time.deltaTime);
		}
	...
	private void UpdateArrowAimBossModeCamera()
	{
		...
		Vector3 b = Quaternion.AngleAxis(axis: Vector3.Cross(normalForward, Vector3.up), angle: 0f - arrowAimSettings.targetingPitch) * normalForward * Mathf.Lerp(validSettings.distanceLimit, targetingDistance, num2);
		Vector3 point = arrowAimSettings.targetingOffset;
		if (arrowCameraMode == 1)
		{
			point = ((!targetSelf.IsAbleArrowSitShot()) ? ((arrowAimStartSign <= 0) ? arrowAimSettings.targetingOffset : arrowAimSettings.targetingLeftOffset) : ((arrowAimStartSign <= 0) ? arrowAimSettings.targetingAvoidShotRightOffset : arrowAimSettings.targetingAvoidShotLeftOffset));
		}
	...
	private void UpdateGrabCamera()
	{
		...
		Vector3 movePosition2 = Vector3.SmoothDamp(maxSpeed: (grabInfo.smoothMaxSpeed > 0f) ? grabInfo.smoothMaxSpeed : validSettings.targetingDistance, current: movePosition, target: requestPos, currentVelocity: ref posVelocity, smoothTime: smoothTargetingRate, deltaTime: Time.deltaTime);
		Vector3 zero = Vector3.zero;
		Vector3 eulerAngles = moveRotation.eulerAngles;
		Vector3 eulerAngles2 = requestRot.eulerAngles;
	...
	private void UpdateStopCamera()
	{
		...
		zero.x = Mathf.SmoothDampAngle(eulerAngles.x, vector.x, ref rotVelocity.x, smoothTargetingRate, num, Time.deltaTime);
		zero.y = Mathf.SmoothDampAngle(eulerAngles.y, vector.y, ref rotVelocity.y, smoothTargetingRate, num, Time.deltaTime);
		zero.z = Mathf.SmoothDampAngle(eulerAngles.z, vector.z, ref rotVelocity.z, smoothTargetingRate, num, Time.deltaTime);
		Quaternion identity = Quaternion.identity;
		identity.eulerAngles = zero;
		this.movePosition = movePosition2;
		this.moveRotation = identity;
	...
	private void UpdateShake()
	{
		...
		while (num < shakeParams.Count)
		{
			...
			if (num3 != num4)
			{
				shakeParam.shakeLength *= Mathf.Pow(shakeAttenuationPercent, num4 - num3);
			...
			zero += Vector3.up * (shakeParam.shakeLength * Mathf.Sin(shakeParam.shakeTime * (float)Math.PI * 2f / num2));
			num++;
		}
		cameraTransform.position = movePosition + zero;
		cameraTransform.rotation = moveRotation;
	}

	private void LateUpdate()
	{
		if (radialBlurFilter != null && radialBlurFilter.enabled)
		{
			UpdateRadialBlur();
		}
		if (!(target == null))
	...
	public void SetMotionCamera(bool enable, Transform target_transform = null, UnityEngine.Object[] cameras = null, Vector3[] camera_offsets = null)
	{
		if (enable)
		{
			...
			Transform transform = Utility.CreateGameObject("FieldQuestCamera", base._transform);
			transform.position = target_transform.position;
			transform.rotation = target_transform.rotation;
			Vector3 one = Vector3.one;
			one.x = target_transform.lossyScale.x / base._transform.lossyScale.x;
			one.y = target_transform.lossyScale.y / base._transform.lossyScale.y;
			one.z = target_transform.lossyScale.z / base._transform.lossyScale.z;
			transform.localScale = one;
			motionCameraParent = transform;
			for (int i = 0; i < 2; i++)
			{
				Transform transform2 = Utility.CreateGameObject("offset_" + i.ToString(), transform);
				...
				Transform transform3 = ResourceUtility.Realizes(cameras[i], transform2);
				if (transform3 == null)
				{
