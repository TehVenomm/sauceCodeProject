	...
	internal class AndroidTokenClient : TokenClient
	{
		...
		public void Signout()
		{
			...
			PlayGamesHelperObject.RunOnGameThread(delegate
			{
				...
				new AndroidJavaClass("com.google.games.bridge.TokenFragment").CallStatic("signOut");
			});
		}

		public bool NeedsToRun()
		{
			if ((!requestAuthCode || !string.IsNullOrEmpty(authCode)) && (!requestEmail || !string.IsNullOrEmpty(email)))
			{
				if (requestIdToken)
				{
			...
			try
			{
				using (AndroidJavaClass androidJavaClass = new AndroidJavaClass("com.google.games.bridge.TokenFragment"))
				{
					using (AndroidJavaObject androidJavaObject = GetActivity())
					{
						...
						array[4].l = AndroidJNI.NewStringUTF(webClientId);
						array[5].z = forceRefresh;
						new PendingResult<TokenResult>(AndroidJNI.CallStaticObjectMethod(androidJavaClass.GetRawClass(), staticMethodID, array)).setResultCallback(new TokenResultCallback(callback));
					}
				}
			}
			catch (Exception ex)
