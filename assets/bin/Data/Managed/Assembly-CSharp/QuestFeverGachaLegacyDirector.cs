...
public class QuestFeverGachaLegacyDirector : QuestGachaDirectorBase
{
	...
	private IEnumerator DoQuestGacha()
	{
		...
		int reamMax = (currentResult == null) ? 9 : currentResult.reward.Count;
		int[] idxList = new int[reamMax];
		for (int k = 0; k < reamMax - 1; k++)
		{
			idxList[k] = k;
		}
		...
		if (MonoBehaviourSingleton<GachaManager>.IsValid() && currentResult != null)
		{
			...
			int l = 0;
			for (int num = count; l < num; l++)
			{
				GachaResult.GachaReward gachaReward = currentResult.reward[l];
				uint itemId = (uint)gachaReward.itemId;
				quest_datas[l] = Singleton<QuestTable>.I.GetQuestData(itemId);
				if (quest_datas[l] != null)
				{
					enemy_datas[l] = Singleton<EnemyTable>.I.GetEnemyData((uint)quest_datas[l].GetMainEnemyID());
					if (enemy_datas[l] == null)
					{
						Log.Error("EnemyTable[{0}] == null", quest_datas[l].GetMainEnemyID());
					}
				}
				else
				{
					Log.Error("QuestTable[{0}] == null", gachaReward.itemId);
					quest_datas[l] = new QuestTable.QuestTableData();
					enemy_datas[l] = new EnemyTable.EnemyData();
				}
			}
		}
		...
		int m = 0;
		for (int num2 = reamMax; m < num2; m++)
		{
			float displayScale = enemy_datas[m].modelScale;
			int anim_id = enemy_datas[m].animId;
			int modelId = enemy_datas[m].modelId;
			OutGameSettingsManager.EnemyDisplayInfo enemyDisplayInfo = MonoBehaviourSingleton<OutGameSettingsManager>.I.SearchEnemyDisplayInfoForGacha(enemy_datas[m]);
			if (enemyDisplayInfo != null)
			{
				anim_id = enemyDisplayInfo.animID;
				displayScale = enemyDisplayInfo.gachaScale;
			}
			enemyLoaderList[m] = LoadEnemy(enemyPositions[idxList[m]], modelId, anim_id, displayScale, enemy_datas[m].baseEffectName, enemy_datas[m].baseEffectNode);
		}
		int j = 0;
		...
		{
			CacheEnemyAudio(enemy_datas[n], lo_queue);
		}
		...
		while (meteor_step < reamMax || magic_step < reamMax)
		{
			time += Time.get_deltaTime();
			if (meteor_step < reamMax && meteorTimings[idxList[meteor_step]] <= time)
			{
				PlayEffect(magicCircles[idxList[meteor_step]], meteorEffectPrefabs[quest_datas[meteor_step].rarity.ToRarityExpressionID()]);
				if (meteor_step == reamMax - 1)
				{
					PlayAudio(AUDIO.METEOR_01);
				}
				meteor_step++;
			}
			if (magic_step < reamMax && magicTimings[idxList[magic_step]] <= time)
			{
				magic_effects[magic_step] = PlayEffect(magicCircles[idxList[magic_step]], magicEffectPrefabs[quest_datas[magic_step].rarity.ToRarityExpressionID()]);
		...
		while (true)
		{
			...
			end_effects[end_step] = PlayEffect(magicCircles[idxList[end_step]], endEffectPrefabs[effect_rarity]);
			Play($"MainAnim_End_{idxList[end_step] + 1:D2}");
			end_step++;
