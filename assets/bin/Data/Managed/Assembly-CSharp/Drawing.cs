...
public class Drawing
{
	...
	public static void DrawLine(Camera cam, Vector3 pointA, Vector3 pointB, Color color, float width)
	{
		...
		Vector3 vector = cam.WorldToScreenPoint(pointA);
		zero.x = vector.x;
		Vector3 vector2 = cam.WorldToScreenPoint(pointA);
		zero.y = vector2.y;
		Vector2 zero2 = Vector2.zero;
		Vector3 vector3 = cam.WorldToScreenPoint(pointB);
		zero2.x = vector3.x;
		Vector3 vector4 = cam.WorldToScreenPoint(pointB);
		zero2.y = vector4.y;
	...
	public static void DrawLine(Vector2 pointA, Vector2 pointB, Color color, float width)
	{
		...
		float angle = Mathf.Atan2(pointB.y - pointA.y, pointB.x - pointA.x) * 180f / 3.14159274f;
