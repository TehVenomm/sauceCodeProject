...
public class QuestGetReward : GameSection
{
	...
	public override void UpdateUI()
	{
		if (reward == null)
		{
			SetActive(UI.OBJ_FRAME, false);
		}
		else
		{
			SetFullScreenButton(UI.BTN_CENTER);
			SetActive(UI.OBJ_FRAME, true);
			...
			{
				ItemIcon itemIcon = ItemIcon.Create(data[i].icon_type, data[i].icon_id, data[i].rarity, t, data[i].element, data[i].magi_enable_equip_type, data[i].num, "REWARD", i, false, -1, false, null, false, data[i].enemy_icon_id, data[i].enemy_icon_id2, false, data[i].getType, ELEMENT_TYPE.MAX);
