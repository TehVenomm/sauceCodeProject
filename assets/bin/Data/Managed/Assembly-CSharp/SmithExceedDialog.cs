...
public class SmithExceedDialog : GameSection
{
	...
	public override void Initialize()
	{
		...
		if (num == 0)
		{
			SetActive(UI.OBJ_SELECT, false);
		}
		else if (num <= 1)
		{
			SetActive(UI.BTN_AIM_R, false);
			SetActive(UI.BTN_AIM_L, false);
		}
		else
		{
			SetActive(UI.BTN_AIM_R_INACTIVE, false);
			SetActive(UI.BTN_AIM_L_INACTIVE, false);
		}
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						ItemIcon.GetIconShowData(REWARD_TYPE.ITEM, exceedNeedItem.itemId, out int _, out ITEM_ICON_TYPE icon_type, out RARITY_TYPE? _, out ELEMENT_TYPE _, out ELEMENT_TYPE _, out EQUIPMENT_TYPE? _, out int _, out int _, out GET_TYPE _, 0);
						Transform transform = FindCtrl(t, UI.OBJ_MATERIAL_ICON_ROOT);
						bool flag2 = haveingItemNum >= num;
						ItemIcon itemIcon = ItemIconMaterial.CreateMaterialIcon(icon_type, itemData, transform, haveingItemNum, num, (!flag2) ? "NEED" : "NEXT", i, false);
						...
						if ((UnityEngine.Object)itemIconMaterial != (UnityEngine.Object)null)
						{
						...
						{
							if ((UnityEngine.Object)itemIcon.GetComponent<UINoAuto>() == (UnityEngine.Object)null)
							{
								itemIcon.gameObject.AddComponent<UINoAuto>();
							}
							if ((UnityEngine.Object)itemIcon.GetComponent<UIButtonScale>() == (UnityEngine.Object)null)
							{
