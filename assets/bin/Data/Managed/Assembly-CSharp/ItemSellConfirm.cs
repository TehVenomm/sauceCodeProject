...
public class ItemSellConfirm : GameSection
{
	...
	public override void UpdateUI()
	{
		...
		if (isButtonSingle)
		{
			SetActive(UI.BTN_CENTER, is_visible: true);
			SetActive(UI.BTN_0, is_visible: false);
			SetActive(UI.BTN_1, is_visible: false);
		}
		else
		{
	...
	protected virtual void DrawIcon()
	{
		...
		SetGrid(UI.GRD_ICON, null, sELL_SELECT_MAX, reset: false, delegate(int i, Transform t, bool is_recycle)
		{
			if (i < sell_data_ary.Length)
			{
				...
				{
					ItemSortData obj = sortCompareData as ItemSortData;
				...
				ItemIcon itemIcon = ItemIcon.Create(sell_data_ary[i].GetIconType(), sell_data_ary[i].GetIconID(), sell_data_ary[i].GetRarity(), t, sell_data_ary[i].GetIconElement(), sell_data_ary[i].GetIconMagiEnableType(), GetTargetIconNum(sell_data_ary, i), null, 0, is_new: false, -1, is_select: false, null, is_equipping, enemy_icon_id, enemy_icon_id2, disable_rarity_text: false, sell_data_ary[i].GetGetType(), sell_data_ary[i].GetIconElementSub());
