...
public class EquipSetDetailAbilityData : GameSection
{
	...
	public override void UpdateUI()
	{
		...
		else if (table != null)
		{
			...
			SetActive(UI.LBL_DESCRIPTION, true);
		...
		if (data != null)
		{
			SetGrid(UI.GRD_DATA_LIST, "EquipSetDetailAbilityDataItem", data.Length, false, delegate(int i, Transform t, bool is_recycle)
			{
				...
				Color color = (ability.ap != -1) ? Color.gray : Color.white;
				if (flag)
				{
					centeringIndex = i;
					color = Color.white;
			...
			if ((Object)component != (Object)null && component.enabled)
			{
				...
				Transform transform = component.transform.FindChild("_DRAG_SCROLL_");
				...
				Vector3 position3 = component2.transform.position;
				Vector3 b = position3 - position;
