...
public class AccessoryTable : Singleton<AccessoryTable>, IDataTable
{
	...
	{
		dataTable = TableUtility.CreateUIntKeyTable<AccessoryData>(text, AccessoryData.cb, "accessoryId,rarity,name,orderValue,attachPlaceBit,getType,price,cantSell,detailScale", null);
		dataTable.TrimExcess();
	}

	public void CreateInfoTable(string text)
	{
		infoTable = TableUtility.CreateUIntKeyTable<AccessoryInfoData>(text, AccessoryInfoData.cb, "id,accessoryId,attachPlace,node,px,py,pz,rx,ry,rz,sx,sy,sz", null);
