...
public class WaveMatchDropResource
{
	...
	public void Create(Coop_Model_WaveMatchDrop model)
	{
		...
		for (int i = 0; i < model.fiIds.Count; i++)
		{
			...
			if (data != null && dropModelList.ContainsKey(data.model))
			{
				...
				OnCreate(MonoBehaviourSingleton<StageObjectManager>.I.waveMatchDropObjIndex++, num, new Vector3(model.x, 0f, model.z), offset, model.sec, send: true);
			}
		}
	}
	...
	public void OnCreate(int manageId, uint dataId, Vector3 basePos, Vector3 offset, float sec, bool send = false)
	{
		...
		Transform transform = ResourceUtility.Realizes(loadObject.loadedObject, MonoBehaviourSingleton<StageObjectManager>.I._transform);
		if (transform == null)
		{
			return;
		}
