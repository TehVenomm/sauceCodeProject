...
public class JackportNumber : MonoBehaviour
{
	...
	public void ShowNumber(string strValue)
	{
		...
		{
			dotList[j].gameObject.SetActive(false);
		}
		int num3 = 0;
		int num4 = 0;
		for (int k = 0; k < length; k++)
		{
			numberList[k].transform.localPosition = new Vector3((float)(-num3), 0f, 0f);
			numberList[k].height = sprHeight;
			if (k % 3 == 2 && k < length - 1)
			{
				num3 += numberList[k].width;
				dotList[num4].gameObject.SetActive(true);
				dotList[num4].localPosition = new Vector3((float)(-num3), (float)(-sprHeight + spaceWidth), 0f);
				num4++;
				num3 += spaceWidth;
			}
			else
