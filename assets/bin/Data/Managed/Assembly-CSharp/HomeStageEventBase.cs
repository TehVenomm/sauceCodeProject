...
public class HomeStageEventBase : MonoBehaviour
{
	...
	public void DispatchEvent()
	{
		if (!HomeBase.OnAfterGacha2Tutorial || eventName.Contains("QUEST_COUNTER"))
		{
			...
			{
				if ((TutorialStep.HasFirstDeliveryCompleted() && !TutorialStep.HasAllTutorialCompleted()) || MonoBehaviourSingleton<UIManager>.I.IsEnableTutorialMessage() || (Object)TutorialMessage.GetCursor(0) != (Object)null)
				{
					return;
				}
			}
			else if (!TutorialStep.HasAllTutorialCompleted() || MonoBehaviourSingleton<UIManager>.I.IsEnableTutorialMessage() || (Object)TutorialMessage.GetCursor(0) != (Object)null)
			{
				return;
			}
			if (eventName.Contains("EVENT_COUNTER") && (int)MonoBehaviourSingleton<UserInfoManager>.I.userStatus.level < 20)
			{
				MonoBehaviourSingleton<GameSceneManager>.I.ExecuteSceneEvent("HomeStageEventBase", base.gameObject, "QUEST_LOCK", null, null, true);
			}
			...
			{
				MonoBehaviourSingleton<GameSceneManager>.I.ExecuteSceneEvent("HomeStageEventBase", base.gameObject, eventName, eventData, null, true);
			}
		}
	}
}
