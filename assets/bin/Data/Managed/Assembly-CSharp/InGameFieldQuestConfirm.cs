		...
		if (desc != null)
		{
			...
			if (questData != null)
			{
				...
				if (enemyData != null)
				{
					...
					{
						SetActive(UI.STR_WEAK_NONE, false);
					}
					...
					PlayTween(UI.TWN_DIFFICULT_STAR, true, null, false, 0);
					QuestInfoData.Mission[] array2 = QuestInfoData.CreateMissionData(questData);
					if (array2 != null)
					{
						GetCtrl(UI.MISSION).gameObject.SetActive(true);
						UI[] array3 = new UI[3]
						{
							UI.MISSION_LABEL_01,
							UI.MISSION_LABEL_02,
					...
					Update();
					UpdateAnchors();
					PlayTween(UI.OBJ_FRAME, true, null, true, 0);
					PlayTween(UI.COUNT_ANIM_0, true, null, false, 0);
				}
			}
		}
	}
	...
	private void Update()
	{
		if (!isAnswer)
		{
			...
			case 0:
				if (num < 10)
				{
					ResetTween(UI.COUNT_ANIM_0, 0);
					PlayTween(UI.COUNT_ANIM_1, true, null, false, 0);
					countAnimStep++;
				}
				break;
			case 1:
				if (num < 6)
				{
					ResetTween(UI.COUNT_ANIM_1, 0);
					PlayTween(UI.COUNT_ANIM_2, true, null, false, 0);
