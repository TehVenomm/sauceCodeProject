...
public class ChatHeaderButtonController : MonoBehaviour
{
	...
	private void InitDepth(int _index)
	{
		...
		{
			m_backGroundSprite.depth = _index * 3;
		}
		if (m_bgSprite != null)
		{
	...
	public void InitUILabel(MainChat.CHAT_TYPE _t)
	{
		if (!(m_buttonLabel == null))
		{
			...
			{
				m_buttonLabel.fontSize = 16;
			}
			else
			{
				m_buttonLabel.fontSize = 12;
			}
		}
	}
	...
	private void SetBgSprite(int _index)
	{
		if (!(m_backGroundSprite == null))
		{
			...
			if (flag2)
			{
				...
				ButtonWidget.rightAnchor.Set(1f, -44f);
			}
			else if (flag)
			{
	...
	public void Deactivate()
	{
		if (IsValidObjects())
		{
			...
			SetBgSprite(ButtonIndex);
			SetDepth();
			m_buttonObject.SetState(UIButtonColor.State.Disabled, immediate: true);
