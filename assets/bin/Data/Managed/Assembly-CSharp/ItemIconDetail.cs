...
public class ItemIconDetail : ItemIcon
{
	...
	{
		ItemIconDetail itemIconDetail = ItemIcon.CreateIcon<ItemIconDetail>(MonoBehaviourSingleton<GlobalSettingsManager>.I.linkResources.itemIconDetailPrefab, ITEM_ICON_TYPE.NONE, ItemIcon.GetRemoveButtonIconID(), null, parent, ELEMENT_TYPE.MAX, null, -1, event_name, event_data, is_new: false, toggle_group, is_select);
		itemIconDetail.setupperRemoveBtn.Set(new object[1]
		{
			name
		});
		itemIconDetail.SetFavoriteIcon(is_favorite: false);
	...
	{
		ItemIconDetail itemIconDetail = ItemIcon.CreateIcon<ItemIconDetail>(MonoBehaviourSingleton<GlobalSettingsManager>.I.linkResources.itemIconDetailPrefab, icon_type, icon_id, rarity, parent, ELEMENT_TYPE.MAX, null, -1, event_name, event_data, is_new, toggle_group, is_select, string.Empty, is_equipping: false, item_table.enemyIconID, item_table.enemyIconID2);
	...
	public static ItemIcon CreateSkillDetailSelectNumberIcon(ITEM_ICON_TYPE icon_type, int icon_id, RARITY_TYPE? rarity, SkillItemSortData item_data, bool is_show_main_status, Transform parent = null, string event_name = null, int event_data = 0, bool is_new = false, int toggle_group = -1, int select_number = -1, bool is_equipping = false, ICON_STATUS icon_status = ICON_STATUS.NONE, bool isSameSkillExceed = false)
	{
		ItemIcon itemIcon = _CreateSkillDetailIcon(icon_type, icon_id, rarity, item_data, is_show_main_status, parent, event_name, event_data, icon_status, is_new, toggle_group, select_number, is_equipping, isSameSkillExceed);
		itemIcon.SetFavoriteIcon(item_data.IsFavorite());
		return itemIcon;
	}

	private static ItemIcon _CreateSkillDetailIcon(ITEM_ICON_TYPE icon_type, int icon_id, RARITY_TYPE? rarity, SkillItemSortData item_data, bool is_show_main_status, Transform parent = null, string event_name = null, int event_data = 0, ICON_STATUS icon_status = ICON_STATUS.NONE, bool is_new = false, int toggle_group = -1, int select_number = -1, bool is_equipping = false, bool isSameSkillExceed = false)
	{
		ItemIconDetail itemIconDetail = ItemIcon.CreateIcon<ItemIconDetail>(MonoBehaviourSingleton<GlobalSettingsManager>.I.linkResources.itemIconDetailPrefab, icon_type, icon_id, rarity, parent, item_data.GetIconElement(), item_data.skillData.tableData.GetEnableEquipType(), -1, event_name, event_data, is_new, toggle_group, select_number > -1, string.Empty, is_equipping, 0, 0, disable_rarity_text: false, QUEST_ICON_SIZE_TYPE.DEFAULT, GET_TYPE.PAY, item_data.GetIconElementSub());
		itemIconDetail.setupperSkill.Set(new object[5]
		{
			item_data,
			is_show_main_status,
			select_number,
	...
	{
		ItemIconDetail itemIconDetail = ItemIcon.CreateIcon<ItemIconDetail>(MonoBehaviourSingleton<GlobalSettingsManager>.I.linkResources.itemIconDetailPrefab, icon_type, icon_id, rarity, parent, quest_item.GetEnemyElement(), null, -1, event_name, event_data, is_new, toggle_group, is_select, string.Empty);
