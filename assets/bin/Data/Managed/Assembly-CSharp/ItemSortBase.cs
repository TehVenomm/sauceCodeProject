...
public class ItemSortBase : SortBase
{
	...
	public override void UpdateUI()
	{
		...
		{
			SetActive(UI.MATERIAL_ROOT, true);
			SetActive(UI.RARITY_ROOT, false);
			SetActive(UI.EQUIP_FILTER_ROOT, false);
			SetActive(UI.ELEMENT_ROOT, true);
			int i = 0;
			...
			Transform transform = GameObject.Find("sort").gameObject.transform;
			transform.localPosition = new Vector3(0f, -322f, 0f);
		}
		else if (sortOrder.dialogType == DIALOG_TYPE.SMITH_CREATE_WEAPON || sortOrder.dialogType == DIALOG_TYPE.SMITH_CREATE_ARMOR)
		{
			SetActive(UI.MATERIAL_ROOT, false);
			SetActive(UI.EQUIP_FILTER_ROOT, true);
			SetActive(UI.RARITY_ROOT, true);
			SetActive(UI.ELEMENT_ROOT, true);
			int k = 0;
			...
			UIWidget component2 = GameObject.Find("ItemSortFrame").gameObject.GetComponent<UIWidget>();
			Transform transform2 = component2.transform;
			Vector3 localPosition = transform2.localPosition;
			float x = localPosition.x;
			Vector3 localPosition2 = transform2.localPosition;
			float y = localPosition2.y + 40f;
			Vector3 localPosition3 = transform2.localPosition;
			Vector3 vector2 = transform2.localPosition = new Vector3(x, y, localPosition3.z);
			component2.height = 700;
			Transform transform3 = GameObject.Find("sort").gameObject.transform;
			Vector3 localPosition4 = transform3.localPosition;
			float x2 = localPosition4.x;
			Vector3 localPosition5 = transform3.localPosition;
			float y2 = localPosition5.y - 185f;
			Vector3 localPosition6 = transform3.localPosition;
			Vector3 vector4 = transform3.localPosition = new Vector3(x2, y2, localPosition6.z);
		}
		else if (sortOrder.dialogType == DIALOG_TYPE.ABILITY_ITEM)
		{
			...
			UIWidget component3 = GameObject.Find("ItemSortFrame").gameObject.GetComponent<UIWidget>();
			component3.height = 583;
			GetCtrl(UI.ELEMENT_ROOT).localPosition = new Vector3(0f, -98f, 0f);
			Transform transform4 = GameObject.Find("sort").gameObject.transform;
			transform4.localPosition = new Vector3(0f, -264f, 0f);
		}
		else
		{
			SetActive(UI.MATERIAL_ROOT, false);
			SetActive(UI.EQUIP_FILTER_ROOT, false);
			SetActive(UI.RARITY_ROOT, true);
			SetActive(UI.ELEMENT_ROOT, false);
			int num9 = 0;
		...
		{
			UI? nullable2 = requirementButton[num12];
			if (nullable2.HasValue)
			{
				...
				if ((num14 & num11) != 0)
				{
					...
					nullable = requirementButton[num12];
				}
				else
				{
					SetActive(requirementButton[num12], false);
				}
			}
		}
