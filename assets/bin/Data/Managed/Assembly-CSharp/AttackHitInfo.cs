...
public class AttackHitInfo : AttackInfo
{
	...
	public override AttackInfo GetRateAttackInfo(AttackInfo rate_info, float rate)
	{
		...
		attackHitInfo.hitEffectName = ((rate < 1f) ? hitEffectName : attackHitInfo2.hitEffectName);
		attackHitInfo.playCommonHitSe = ((rate < 1f) ? playCommonHitSe : attackHitInfo2.playCommonHitSe);
		attackHitInfo.hitSEID = ((rate < 1f) ? hitSEID : attackHitInfo2.hitSEID);
		attackHitInfo.playCommonHitEffect = ((rate < 1f) ? playCommonHitEffect : attackHitInfo2.playCommonHitEffect);
		attackHitInfo.remainEffectName = ((rate < 1f) ? remainEffectName : attackHitInfo2.remainEffectName);
		...
		attackHitInfo.attackType = ((rate < 1f) ? attackType : attackHitInfo2.attackType);
		attackHitInfo.spAttackType = ((rate < 1f) ? spAttackType : attackHitInfo2.spAttackType);
		attackHitInfo.hitIntervalTime = AttackInfo.GetRateValue(hitIntervalTime, attackHitInfo2.hitIntervalTime, rate);
		attackHitInfo.enableIdentityCheck = ((rate <= 0f) ? enableIdentityCheck : attackHitInfo2.enableIdentityCheck);
		attackHitInfo.isValidTriggerStay = ((rate <= 0f) ? isValidTriggerStay : attackHitInfo2.isValidTriggerStay);
		attackHitInfo.toPlayer.reactionType = ((rate < 1f) ? toPlayer.reactionType : attackHitInfo2.toPlayer.reactionType);
		...
		attackHitInfo.toEnemy.reactionType = ((rate < 1f) ? toEnemy.reactionType : attackHitInfo2.toEnemy.reactionType);
		...
		attackHitInfo.toEnemy.hitTypeName = ((rate < 1f) ? toEnemy.hitTypeName : attackHitInfo2.toEnemy.hitTypeName);
		attackHitInfo.toEnemy.isSpecialAttack = ((rate < 1f) ? toEnemy.isSpecialAttack : attackHitInfo2.toEnemy.isSpecialAttack);
		attackHitInfo.toEnemy.concussion = AttackInfo.GetRateValue(toEnemy.concussion, attackHitInfo2.toEnemy.concussion, rate);
		string[] array = (rate < 1f) ? damageUpLabels : attackHitInfo2.damageUpLabels;
		if (array != null)
		{
