...
public class QuestAcceptAssignedEquipment : SkillInfoBase
{
	...
	private void UpdateEquipIcon()
	{
		...
		for (int num2 = setInfo.item.Length; j < num2; j++)
		{
			...
			{
				SetActive(GetCtrl(iconsBtn[j]), is_visible: false);
				continue;
			}
			...
			if (equipItemInfo != null && equipItemInfo.tableID != 0)
			{
				...
				SetActive(GetCtrl(iconsLevel[j]), is_visible: true);
	...
	private void LoadModel()
	{
		if (record != null)
		{
			...
			{
				record.playerLoadInfo = PlayerLoadInfo.FromUserStatus(need_weapon: true, is_priority_visual_equip: false);
				record.animID = -1;
	...
	private void UpdateStatusUI()
	{
		...
		otherEquipSetCalculator.SetEquipSet(equips);
	...
	private void OnQuery_ABILITY()
	{
		...
		{
			setInfo,
			MonoBehaviourSingleton<StatusManager>.I.GetEquipSetAbility(setInfo),
