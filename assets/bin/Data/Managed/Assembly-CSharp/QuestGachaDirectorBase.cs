...
public class QuestGachaDirectorBase : AnimationDirector
{
	...
	protected void Delete()
	{
		...
		effects.ForEach(delegate(Transform o)
		{
			if (o != null)
			{
				EffectManager.ReleaseEffect(o.gameObject);
			}
		});
		effects.Clear();
	}
	...
	{
		NPCLoader nPCLoader = Utility.CreateGameObject("NPC", npcPosition).gameObject.AddComponent<NPCLoader>();
		nPCLoader.Load(Singleton<NPCTable>.I.GetNPCData(2).npcModelID, 0, need_shadow: false, enable_light_probes: true, SHADER_TYPE.NORMAL, null);
	...
	{
		EnemyLoader enemyLoader = Utility.CreateGameObject("Enemy", parent).gameObject.AddComponent<EnemyLoader>();
		enemyLoader.StartLoad(model_id, anim_id, 1f, base_effect, base_effect_node, need_shadow: false, enable_light_probes: true, need_anim_event_res_cache: false, SHADER_TYPE.NORMAL);
	...
	protected void PlayMagicEffect(int rarity, bool rankup)
	{
		...
		Play("MainAnim_MagicAngle0" + rarity);
	}
	...
	{
		PlayAudio(audio, ignore_skip: false);
	}
	...
	{
		AUDIO audio = is_short ? AUDIO.APEAR_SHORT : AUDIO.APEAR_LONG;
		PlayAudio(audio);
	}
	...
	protected void PlayEnemyAudio(EnemyTable.EnemyData enemyData, bool is_short = false)
	{
		...
		if (enemyDisplayInfo != null)
		{
			int num = is_short ? enemyDisplayInfo.seIdGachaShort : enemyDisplayInfo.seIdGachaLong;
			if (num > 0)
			{
				SoundManager.PlayOneshotJingle(num);
			}
		}
	}
