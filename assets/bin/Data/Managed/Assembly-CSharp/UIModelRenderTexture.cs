...
public class UIModelRenderTexture : MonoBehaviour
{
	...
	public void ForceInitPlayer(UITexture ui_tex, PlayerLoadInfo info, int anim_id, Vector3 pos, Vector3 rot, bool is_priority_visual_equip, Action<PlayerLoader> onload_callback)
	{
		...
		bool num = IsLoadingPlayer();
		bool flag = npcLoader != null;
		if (num | flag)
		{
			if (flag)
			{
	...
	public void InitNPC(UITexture ui_tex, int npc_id, Vector3 pos, Vector3 rot, float fov, Action<NPCLoader> onload_callback)
	{
		...
		if (nPCData != null && nPCData != npcData)
		{
			...
			Init(ui_tex, (fov != -1f) ? fov : 10f, LOADER_TYPE.NPC);
			...
			int num = (nPCData.specialModelID > 0) ? nPCData.specialModelID : nPCData.npcModelID;
	...
	public void InitEnemy(UITexture ui_tex, uint enemy_id, string foundation_name, OutGameSettingsManager.EnemyDisplayInfo.SCENE target_scene, Action<bool, EnemyLoader> callback = null, ENEMY_MOVE_TYPE moveType = ENEMY_MOVE_TYPE.DEFULT, bool is_Howl = true)
	{
		...
		enemyLoader.StartLoad(enemyData.modelId, anim_id, scale, enemyData.baseEffectName, enemyData.baseEffectNode, need_shadow: false, enable_light_probes: false, need_anim_event_res_cache: true, SHADER_TYPE.UI, modelLayer, foundation_name, need_stamp_effect: false, will_stock: false, "", delegate(Enemy enemy)
		{
			if (callback != null)
			{
		...
		for (int count = Temporary.rendererList.Count; i < count; i++)
		{
			...
			for (int num = materials.Length; j < num; j++)
			{
				...
				else if (CanChangeHighQualityShader(name))
				{
					...
					if (material.HasProperty("_CullMode") && material.GetInt("_CullMode") == 0 && shader != null)
					{
	...
	private void DeleteModel()
	{
		if (model != null)
		{
			...
			{
				npcLoader.Clear();
				UnityEngine.Object.Destroy(npcLoader);
				npcLoader = null;
			}
			if (enemyLoader != null)
			{
	...
	private IEnumerator DoViewing()
	{
		...
		else if (enemyLoader != null)
		{
			if (enemyDispplayInfo == null)
			{
				...
				{
					bounds.Encapsulate(enemyLoader.renderersBody[i].bounds);
				}
				float z = bounds.extents.x * 0.5f / Mathf.Tan((float)Math.PI / 8f) + 1f;
				model.localPosition = new Vector3(0f, bounds.extents.y * -0.5f, z);
			...
			else
			{
				...
				if (animator != null)
				{
					int num2 = Animator.StringToHash("Base Layer.GACHA_HOWL");
					if (animator.HasState(0, num2))
					{
						animator.Play(num2, 0, 0f);
						animator.Update(0f);
					}
				}
			}
	...
	private IEnumerator _PlayRandomEnemyAnimation()
	{
		if (!isRandomPlaying)
		{
			...
			int num = UnityEngine.Random.Range(0, enemyLoader.animEventData.animations.Length);
			enemyLoader.animator.Play(enemyLoader.animEventData.animations[num].name);
			yield return new WaitForSeconds(enemyLoader.animator.GetCurrentAnimatorClipInfo(0)[0].clip.length - 0.5f);
