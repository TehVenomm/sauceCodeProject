	...
	public class ScreenSpaceAmbientOcclusion
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			else
			{
				...
				if (flag)
				{
					RenderTexture val2 = RenderTexture.GetTemporary(source.get_width(), source.get_height(), 0);
					...
					Graphics.Blit(null, val2, m_SSAOMaterial, 3);
					RenderTexture.ReleaseTemporary(val);
					RenderTexture val3 = RenderTexture.GetTemporary(source.get_width(), source.get_height(), 0);
					m_SSAOMaterial.SetVector("_TexelOffsetScale", new Vector4(0f, (float)m_Blur / (float)source.get_height(), 0f, 0f));
					m_SSAOMaterial.SetTexture("_SSAO", val2);
					Graphics.Blit(source, val3, m_SSAOMaterial, 3);
