		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			else
			{
				...
				else
				{
					...
					RenderTexture val = RenderTexture.GetTemporary((int)adaptiveTextureSize, (int)adaptiveTextureSize, 0, rtFormat);
					Graphics.Blit(source, val);
					int num2 = (int)Mathf.Log((float)val.get_width() * 1f, 2f);
					int num3 = 2;
					RenderTexture[] array = (RenderTexture[])new RenderTexture[num2];
					for (int i = 0; i < num2; i++)
					{
						array[i] = RenderTexture.GetTemporary(val.get_width() / num3, val.get_width() / num3, 0, rtFormat);
						num3 *= 2;
					}
					RenderTexture val2 = array[num2 - 1];
					Graphics.Blit(val, array[0], tonemapMaterial, 1);
					...
					Graphics.Blit(val2, rt, tonemapMaterial, (!flag) ? 2 : 3);
