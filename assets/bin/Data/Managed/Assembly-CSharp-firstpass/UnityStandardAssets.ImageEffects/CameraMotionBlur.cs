	...
	public class CameraMotionBlur : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			else
			{
				...
				RenderTexture val2 = RenderTexture.GetTemporary(divRoundUp(source.get_width(), velocityDownsample), divRoundUp(source.get_height(), velocityDownsample), 0, val);
				int num = 1;
				int num2 = 1;
				...
				if (filterType == MotionBlurFilter.Reconstruction || flag || filterType == MotionBlurFilter.ReconstructionDisc)
				{
					...
					num = divRoundUp(val2.get_width(), (int)maxVelocity);
					num2 = divRoundUp(val2.get_height(), (int)maxVelocity);
					num3 = (float)(val2.get_width() / num);
				}
				else
				{
					num = divRoundUp(val2.get_width(), (int)maxVelocity);
				...
				RenderTexture val4 = RenderTexture.GetTemporary(num, num2, 0, val);
				val2.set_filterMode(0);
				val3.set_filterMode(0);
				val4.set_filterMode(0);
				...
				wasActive = this.get_gameObject().get_activeInHierarchy();
				Matrix4x4 val5 = Matrix4x4.Inverse(currentViewProjMat);
				motionBlurMaterial.SetMatrix("_InvViewProj", val5);
				motionBlurMaterial.SetMatrix("_PrevViewProj", prevViewProjMat);
				motionBlurMaterial.SetMatrix("_ToPrevViewProjCombined", prevViewProjMat * val5);
				...
				motionBlurMaterial.SetTexture("_VelTex", val2);
				motionBlurMaterial.SetTexture("_NeighbourMaxTex", val4);
				motionBlurMaterial.SetTexture("_TileTexDebug", val3);
				if (preview)
				{
					Matrix4x4 worldToCameraMatrix = _camera.get_worldToCameraMatrix();
				...
				if (filterType == MotionBlurFilter.CameraMotion)
				{
					...
					Vector3 val6 = prevFramePos - this.get_transform().get_position();
					...
					{
						zero.w = movementScale * Vector3.Dot(this.get_transform().get_forward(), val6) * ((float)source.get_width() * 0.5f);
						zero.x += movementScale * Vector3.Dot(this.get_transform().get_up(), val6) * ((float)source.get_width() * 0.5f);
						zero.y += movementScale * Vector3.Dot(this.get_transform().get_right(), val6) * ((float)source.get_width() * 0.5f);
					}
					if (preview)
					{
				...
				{
					Graphics.Blit(source, val2, motionBlurMaterial, 0);
					Camera val7 = null;
					if (excludeLayers.get_value() != 0)
					{
						val7 = GetTmpCam();
					}
					if (Object.op_Implicit(val7) && excludeLayers.get_value() != 0 && Object.op_Implicit(replacementClear) && replacementClear.get_isSupported())
					{
						val7.set_targetTexture(val2);
						val7.set_cullingMask(LayerMask.op_Implicit(excludeLayers));
						val7.RenderWithShader(replacementClear, string.Empty);
					}
				}
				...
				if (showVelocity)
				{
					...
					Graphics.Blit(val2, destination, motionBlurMaterial, 1);
				}
				else if (filterType == MotionBlurFilter.ReconstructionDX11 && !flag)
				{
					...
					dx11MotionBlurMaterial.SetTexture("_VelTex", val2);
					dx11MotionBlurMaterial.SetTexture("_NeighbourMaxTex", val4);
					...
					Graphics.Blit(val2, val3, dx11MotionBlurMaterial, 0);
					Graphics.Blit(val3, val4, dx11MotionBlurMaterial, 1);
				...
				else if (filterType == MotionBlurFilter.Reconstruction || flag)
				{
					...
					Graphics.Blit(val2, val3, motionBlurMaterial, 2);
					Graphics.Blit(val3, val4, motionBlurMaterial, 3);
