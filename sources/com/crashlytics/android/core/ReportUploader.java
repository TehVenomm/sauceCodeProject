...
class ReportUploader {
    ...
    private class Worker extends BackgroundPriorityRunnable {
        ...
        private void attemptUploadWithRetry() {
            Fabric.getLogger().mo4753d("Fabric", "Starting report processing in " + this.delay + " second(s)...");
            if (this.delay > 0.0f) {
                try {
            ...
            if (!handler.isHandlingException()) {
                if (findReports.isEmpty() || instance.canSendWithUserApproval()) {
                    ...
                    while (!findReports.isEmpty() && !CrashlyticsCore.getInstance().getHandler().isHandlingException()) {
                        Fabric.getLogger().mo4753d("Fabric", "Attempting to send " + findReports.size() + " report(s)");
                        ...
                        if (!findReports2.isEmpty()) {
                            ...
                            Fabric.getLogger().mo4753d("Fabric", "Report submisson: scheduling delayed retry in " + j + " seconds");
                            try {
                                Thread.sleep(j * 1000);
                                i++;
                ...
                Fabric.getLogger().mo4753d("Fabric", "User declined to send. Removing " + findReports.size() + " Report(s).");
        ...
        public void onRun() {
            try {
                ...
                Fabric.getLogger().mo4756e("Fabric", "An unexpected error occurred while attempting to upload crash reports.", e);
            }
    ...
    List<Report> findReports() {
        Fabric.getLogger().mo4753d("Fabric", "Checking for crash reports...");
        ...
        for (File file : listFiles) {
            Fabric.getLogger().mo4753d("Fabric", "Found crash report " + file.getPath());
            linkedList.add(new SessionReport(file));
        }
        if (linkedList.isEmpty()) {
            Fabric.getLogger().mo4753d("Fabric", "No reports found.");
        }
        return linkedList;
    }
    ...
    boolean forceUpload(Report report) {
        ...
        synchronized (this.fileAccessLock) {
            try {
                ...
                Fabric.getLogger().mo4758i("Fabric", "Crashlytics report upload " + (invoke ? "complete: " : "FAILED: ") + report.getFileName());
                if (invoke) {
                    report.remove();
                    z = true;
                } else {
                ...
                Fabric.getLogger().mo4756e("Fabric", "Error occurred sending report " + report, e);
                z = false;
            }
        }
        return z;
