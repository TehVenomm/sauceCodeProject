...
class CrashlyticsBackgroundWorker {
            ...
            return this.executorService.submit(new Runnable() {
                public void run() {
                    try {
                        ...
                        Fabric.getLogger().mo20972e(CrashlyticsCore.TAG, "Failed to execute task.", e);
                    }
                }
            });
        } catch (RejectedExecutionException e) {
            Fabric.getLogger().mo20969d(CrashlyticsCore.TAG, "Executor is shut down because we're handling a fatal crash.");
            return null;
        }
    }
    ...
    public <T> T submitAndWait(Callable<T> callable) {
        try {
            ...
            Fabric.getLogger().mo20972e(CrashlyticsCore.TAG, "Failed to execute task.", e2);
            return null;
        }
    }
}
