...
class BinaryImagesConverter {
    ...
    private File correctDataPath(File file) {
        if (VERSION.SDK_INT < 9 || !file.getAbsolutePath().startsWith(DATA_DIR)) {
            return file;
        }
        try {
            ...
            Fabric.getLogger().mo20972e(CrashlyticsCore.TAG, "Error getting ApplicationInfo", e);
            return file;
        }
    }
    ...
    private static byte[] generateBinaryImagesJsonString(JSONArray jSONArray) {
        ...
        try {
            ...
            Fabric.getLogger().mo20983w(CrashlyticsCore.TAG, "Binary images string is null", e);
            return new byte[0];
        }
    }
    ...
    private static boolean isRelevant(ProcMapEntry procMapEntry) {
        return (procMapEntry.perms.indexOf(DisplaySupport.SCREEN_DENSITY_LOW) == -1 || procMapEntry.path.indexOf(47) == -1) ? false : true;
    }
    ...
    private JSONObject jsonFromMapEntryString(String str) {
        ...
        try {
            try {
                ...
                Fabric.getLogger().mo20970d(CrashlyticsCore.TAG, "Could not create a binary image json string", e);
                return jSONObject;
            }
        } catch (IOException e2) {
            Fabric.getLogger().mo20970d(CrashlyticsCore.TAG, "Could not generate ID for file " + parse.path, e2);
            return jSONObject;
        }
    }
    ...
    private JSONArray parseProcMapsJsonFromString(String str) {
        ...
        try {
            ...
            Fabric.getLogger().mo20983w(CrashlyticsCore.TAG, "Unable to parse proc maps string", e);
        }
        return jSONArray;
    }
