...
class PersistentHitStore implements HitStore {
    ...
    private static final String CREATE_HITS_TABLE = ("CREATE TABLE IF NOT EXISTS hits (" + String.format(" '%s' INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,", new Object[]{HIT_ID}) + String.format(" '%s' TEXT NOT NULL,", new Object[]{HIT_STRING}) + String.format(" '%s' INTEGER NOT NULL);", new Object[]{HIT_TIMESTAMP}));
    ...
    static class DataBaseHelper extends SQLiteOpenHelper {
        ...
        private void fixReferrerTable(SQLiteDatabase sQLiteDatabase) {
            ...
            Cursor cursor = null;
            try {
                Cursor query = sQLiteDatabase.query("referrer", null, null, null, null, null, null);
                try {
                    ...
                    if (i3 == 0 || r0 == 0) {
                        ...
                        if (query.moveToFirst()) {
                            ...
                            referrer = new Referrer(query.getString(query.getColumnIndex("referrer")), query.getLong(query.getColumnIndex(PersistentHitStore.TIMESTAMP_REFERRER)), columnIndex == -1 ? 1 : query.getInt(columnIndex), columnIndex2 == -1 ? 1 : query.getInt(columnIndex2));
                        } else {
                            referrer = null;
                        }
                        ...
                        if (referrer != null) {
                            ...
                            contentValues.put("referrer", referrer.getReferrerString());
    ...
    private void putEvent(Event event, SQLiteDatabase sQLiteDatabase, boolean z) {
        if (!event.isSessionInitialized()) {
            event.setRandomVal(this.random.nextInt(Strategy.TTL_SECONDS_INFINITE));
        ...
        try {
            ...
            try {
                if (query.moveToFirst()) {
                    ...
                    this.storeId = new SecureRandom().nextInt() & Strategy.TTL_SECONDS_INFINITE;
                    query.close();
                    ContentValues contentValues = new ContentValues();
                    ...
                    sQLiteDatabase.insert(SettingsJsonConstants.SESSION_KEY, null, contentValues);
                    query = null;
                }
                if (query != null) {
                    query.close();
    ...
    Referrer readCurrentReferrer(SQLiteDatabase sQLiteDatabase) {
        ...
        try {
            ...
            try {
                Referrer referrer;
                if (query.moveToFirst()) {
                    referrer = new Referrer(query.getString(query.getColumnIndex("referrer")), query.getLong(query.getColumnIndex(TIMESTAMP_REFERRER)), query.getInt(query.getColumnIndex(REFERRER_VISIT)), query.getInt(query.getColumnIndex(REFERRER_INDEX)));
                } else {
                    referrer = null;
                }
                if (query == null) {
